...
class PopulateJsonFireStore {
  ...
  add(item) {
    console.log(`Adding item with id ${item.ISO}`);
    return this.db.collection(this.collectionname).add(Object.assign({}, item))
    .then(() => true)
    .catch((e) => console.error(e.message));
  }

  // Set data with specified ISO
  // Custom Generated ISOS
  set(item) {
    console.log(`setting item with ISO ${item.ISO}`);
    return this.db.doc(`${this.collectionname}/${item.ISO}`).set(Object.assign({}, item))
    .then(() => true)
    .catch((e) => console.error(e.message));
  }
